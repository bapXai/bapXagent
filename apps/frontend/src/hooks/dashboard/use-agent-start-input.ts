...
export function useAgentStartInput(options: UseAgentStartInputOptions = {}): UseAgentStartInputReturn {
  ...
  // Determine if bapX agent is selected (for modes panel)
  // For unauthenticated users, always show as bapX agent (for landing page modes panel)
  const isbapXAgent = !user 
    ? true // Unauthenticated users always see bapX modes
    : isLoadingAgents 
