...
import { usebapXComputerStore, useIsSidePanelOpen, useSetIsSidePanelOpen } from '@/stores/bapx-computer-store';
...
export function useThreadToolCalls(
  ...
  const navigateToToolCall = usebapXComputerStore((state) => state.navigateToToolCall);
  ...
  const handleToolClick = useCallback((clickedAssistantMessageId: string | null, clickedToolName: string, toolCallId?: string) => {
    ...
    const navigateToIndex = (index: number) => {
      ...
      setIsSidePanelOpen(true);
      // Use store action to ensure bapXComputer switches to tools view
