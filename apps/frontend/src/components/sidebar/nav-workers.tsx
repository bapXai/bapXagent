...
export function NavWorkers() {
  ...
  const customAgents = useMemo(() => {
    return filteredAgents.filter(a => !a.metadata?.is_bapx_default);
  }, [filteredAgents]);
    ...
    <div className="flex flex-col h-full pt-4">
      ...
      <div className="flex-1 overflow-y-auto space-y-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
        {isLoading ? (
          ...
          <>
            {bapxAgent && !searchQuery && (
              <div
                className={cn(
                  "flex items-center gap-3 p-2.5 rounded-xl cursor-pointer transition-colors",
                  "hover:bg-muted/50"
                )}
                onClick={() => handleAgentClick(bapxAgent.agent_id)}
              >
                <AgentAvatar 
                  agent={bapxAgent} 
                  agentId={bapxAgent.agent_id} 
                  size={32} 
                  className="flex-shrink-0"
                />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium truncate">{bapxAgent.name}</div>
