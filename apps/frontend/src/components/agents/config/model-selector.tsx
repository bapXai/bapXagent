...
export function AgentModelSelector({
  ...
  const handleSelect = (modelId: string) => {
    ...
    if (hasAccess) {
      ...
      const isPowerModel = modelId === 'bapx/power';
      openPricingModal({
        isAlert: true,
        alertTitle: isPowerModel ? 'Upgrade to access bapX Advanced mode' : 'Upgrade to access this model',
      });
    }
  };
  ...
  const renderModelOption = (model: any, index: number) => {
    ...
    const isPowerModel = model.id === 'bapx/power';
        ...
        <TooltipTrigger asChild>
          ...
          {!accessible && !isLocalMode() ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
              <p>{isPowerModel ? 'Upgrade to access bapX Advanced mode' : 'Upgrade to access this model'}</p>
            </TooltipContent>
          ) : isLowQuality ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
