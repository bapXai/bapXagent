...
export function AgentConfigurationDialog({
  ...
  const isNameEditable = !isViewingOldVersion && (restrictions.name_editable !== false) && !isbapXAgent;
  const isSystemPromptEditable = !isViewingOldVersion && (restrictions.system_prompt_editable !== false) && !isbapXAgent;
  const areToolsEditable = !isViewingOldVersion && (restrictions.tools_editable !== false) && !isbapXAgent;
    ...
    if (!isNameEditable) {
      if (isbapXAgent) {
        toast.error("Name cannot be edited", {
          description: "bapX's name is managed centrally and cannot be changed.",
        });
      }
      setEditName(formData.name);
      setIsEditingName(false);
    ...
    if (!isSystemPromptEditable) {
      if (isbapXAgent) {
        toast.error("System prompt cannot be edited", {
          description: "bapX's system prompt is managed centrally.",
        });
      }
      return;
    }
    ...
    if (!areToolsEditable) {
      if (isbapXAgent) {
        toast.error("Tools cannot be edited", {
          description: "bapX's tools are managed centrally.",
        });
      }
      return;
    }
                              ...
                              {agents.map((agent: any) => (
                                ...
                                >
                                  <AgentAvatar
                                    ...
                                    isbapXDefault={agent.metadata?.is_bapx_default}
                                    size={24}
                                    className="flex-shrink-0"
                                  />
                    ...
                    {isbapXAgent && (
                      <Alert className="mb-4 bg-blue-50 border-blue-200 dark:bg-blue-950/20 dark:border-blue-900">
                        ...
                        <AlertDescription className="text-sm text-blue-800 dark:text-blue-300">
                          You can't edit the main bapX, but you can create a new AI Worker that you can modify as you wish.
                        </AlertDescription>
                      </Alert>
                    )}
