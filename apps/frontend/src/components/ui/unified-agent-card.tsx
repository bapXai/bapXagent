...
const CardAvatar: React.FC<{ 
  ...
  const isbapXAgent = data.metadata?.is_bapx_default === true;
...
const AgentBadges: React.FC<{ data: BaseAgentData, isbapXAgent: boolean }> = ({ data, isbapXAgent }) => (
  <div className="flex gap-1">
    {!isbapXAgent && data.current_version && (
      <Badge variant="outline" className="text-xs">
        <GitBranch className="h-3 w-3 mr-1" />
        {data.current_version.version_name}
      </Badge>
    )}
    {!isbapXAgent && data.is_public && (
...
export const UnifiedAgentCard: React.FC<UnifiedAgentCardProps> = ({
  ...
  const renderStandardCard = () => {
    ...
    const renderBadge = () => {
      switch (variant) {
        case 'marketplace':
          return <MarketplaceBadge isbapXTeam={data.is_bapx_team} isOwner={isOwner} />;
        case 'template':
          return <TemplateBadge isPublic={data.is_public} />;
        case 'agent':
          return <AgentBadges data={data} isbapXAgent={isbapXAgent} />;
        default:
          return null;
      }
    };
