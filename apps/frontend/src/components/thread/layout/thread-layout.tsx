...
import { bapXComputer, ToolCallInput } from '@/components/thread/kortix-computer';
...
export const ThreadLayout = memo(function ThreadLayout({
  ...
  // bapX Computer Store - for handling file open requests
  const { shouldOpenPanel, clearShouldOpenPanel, openFileInComputer, openFileBrowser } = usebapXComputerStore();
  ...
  // Handle file click - now opens in bapX Computer instead of modal
  ...
  const selectedFilePath = usebapXComputerStore((state) => state.selectedFilePath);
