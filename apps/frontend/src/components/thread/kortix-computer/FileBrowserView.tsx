...
import { bapXComputerHeader } from './bapXComputerHeader';
...
interface FileBrowserViewProps {
  ...
  projectId?: string;
  /** 
   * 'default' - Side panel view in bapX Computer (shows all files)
      ...
      if (file.is_dir) {
        ...
        if (isDirectChildOfPresentations(file.path)) {
          ...
          if (isLibraryView && project?.sandbox?.sandbox_url) {
            ...
            // In side panel view, use bapx computer store
            openFile(file.path);
          }
        } else {
        ...
        // Open file using bapx computer store
      ...
      <div className="flex flex-col min-h-screen bg-background">
        ...
        <Dialog open={revertModalOpen} onOpenChange={setRevertModalOpen}>
          <DialogContent className="sm:max-w-md rounded-xl bg-background border border-border">
            ...
            {revertLoadingInfo ? (
              <div className="py-6 flex items-center justify-center"><BapXLoader size="medium" /></div>
            ) : revertCommitInfo ? (
              <div className="mt-2">
