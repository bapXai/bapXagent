...
import { 
  ...
  usebapXComputerPendingToolNavIndex,
  usebapXComputerClearPendingToolNav,
} from '@/stores/bapx-computer-store';
...
const FLOATING_LAYOUT_ID = 'bapx-computer-float';

export const bapXComputer = memo(function bapXComputer({
  isOpen,
  onClose,
  toolCalls,
  currentIndex,
  ...
  const pendingToolNavIndex = usebapXComputerPendingToolNavIndex();
  const clearPendingToolNav = usebapXComputerClearPendingToolNav();
  ...
  useEffect(() => {
    ...
    if (projectChanged || sandboxChanged) {
      console.log('[bapXComputer] Project or sandbox changed, resetting local state', { projectId, sandboxId });
  ...
  if (isMobile) {
    const handleDrawerKeyDown = (e: React.KeyboardEvent) => {
      ...
      // Prevent Escape / Esc from closing the bapX Computer.
    ...
    return (
      <Drawer
        ...
        // Never allow Esc/Escape to dismiss the bapX Computer.
