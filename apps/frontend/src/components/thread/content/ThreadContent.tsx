...
// Reusable agent header - shows bapX logo for bapX, avatar+name for others
const AgentHeader = memo(function AgentHeader({ agentInfo }: { agentInfo: AgentInfo }) {
  if (agentInfo.name === "bapX") {
    return (
      <img
...
const AssistantGroupRow = memo(function AssistantGroupRow({
  ...
  return (
    <div key={group.key} ref={isLastGroup ? latestMessageRef : null}>
      <div className="flex flex-col gap-2">
        {/* Reasoning section with integrated bapX icon */}
        {reasoningSection}
  ...
  function ThreadContent({
    ...
    const agentInfo = useMemo<AgentInfo>(() => {
      ...
      if (recentAssistantWithAgent?.agents?.name) {
        ...
        const name =
          typeof rawName === "string" ? rawName : String(rawName || "bapX");
        return {
          name,
          avatar: (
            <div className="h-5 w-5 flex items-center justify-center rounded text-xs">
      ...
      const fallbackName = typeof agentName === "string" ? agentName : "bapX";
      return {
        name: fallbackName || "bapX",
        avatar: agentAvatar,
      };
