...
const LoggedInMenu: React.FC<UnifiedConfigMenuProps> = memo(function LoggedInMenu({
    ...
    const bapxAgent = useMemo(() => {
        return agents.find(a => a.metadata?.is_bapx_default === true);
    }, [agents]);

    const placeholderbapXAgent = useMemo(() => ({
        agent_id: undefined,
        name: 'bapX',
        metadata: { is_bapx_default: true }
    }), []);
    ...
    const displayAgent = useMemo(() => {
        ...
        if (bapxAgent) return bapxAgent;
        return agents[0];
    }, [agents, selectedAgentId, bapxAgent]);
    ...
    const renderAgentIcon = useCallback((agent: any, size: number = 32) => {
        if (!agent && (isLoading || bapxAgent)) {
            return <AgentAvatar isbapXDefault={true} agentName="bapX" size={size} className="flex-shrink-0 !border-0" />;
        }
    ...
    // Check if current agent is bapX (used in multiple places)
    const isbapXAgent = !displayAgent?.name || displayAgent?.name === 'bapX';
                        ...
                        <div className="px-4 pb-2">
                            ...
                            >
                                {isbapXAgent ? (
                                    ...
                                    <div className="flex items-center justify-center w-10 h-10 bg-card border-[1.5px] border-border flex-shrink-0" style={{ borderRadius: '10.4px' }}>
                                        {renderAgentIcon(isLoading && !displayAgent ? placeholderbapXAgent : displayAgent, 40)}
                                    </div>
                                )}
                                <span className="flex-1 truncate text-base font-medium text-left min-w-0">
                                    {isbapXAgent ? 'bapX' : displayAgent?.name}
                                </span>
    ...
    }, [mobileSection, searchQuery, onAgentSelect, displayAgent, isLoading, placeholderbapXAgent, renderAgentIcon, selectedAgentId, AgentsList, CreateWorkerButton, WorkerSettingsButtons, isbapXAgent]);
        ...
        >
            {onAgentSelect ? (
                <div className="flex items-center gap-2 min-w-0 max-w-[180px]">
                    {renderAgentIcon(isLoading && !displayAgent ? placeholderbapXAgent : displayAgent, 24)}
                            ...
                            {onAgentSelect && (
                                <>
                                    ...
                                    <div className="px-2 pb-2">
                                        <SpotlightCard className={cn("transition-colors cursor-pointer bg-transparent", isbapXAgent ? "p-2" : "")}>
                                            <DropdownMenuSub>
                                                <DropdownMenuSubTrigger className="flex items-center gap-3 text-sm cursor-pointer px-1 hover:bg-transparent focus:bg-transparent data-[state=open]:bg-transparent w-full">
                                                    <div className="flex items-center gap-3">
                                                        {renderAgentIcon(isLoading && !displayAgent ? placeholderbapXAgent : displayAgent)}
