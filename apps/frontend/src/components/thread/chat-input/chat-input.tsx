...
function ModeButton({
  ...
  const selectedMode = usebapXModesStore((state) => state.selectedMode);
  const selectedCharts = usebapXModesStore((state) => state.selectedCharts);
  const selectedOutputFormat = usebapXModesStore((state) => state.selectedOutputFormat);
  const selectedTemplate = usebapXModesStore((state) => state.selectedTemplate);
  const selectedDocsType = usebapXModesStore((state) => state.selectedDocsType);
  const selectedImageStyle = usebapXModesStore((state) => state.selectedImageStyle);
  const selectedCanvasAction = usebapXModesStore((state) => state.selectedCanvasAction);
  const selectedVideoStyle = usebapXModesStore((state) => state.selectedVideoStyle);
...
// bapX agent modes switcher - isolated from typing state
interface bapXAgentModeSwitcherProps {
  enabled: boolean;
  isbapXAgent: boolean;
  bapxAgentModes: 'adaptive' | 'autonomous' | 'chat';
  onModeChange: (mode: 'adaptive' | 'autonomous' | 'chat') => void;
}

const bapXAgentModeSwitcher = memo(function bapXAgentModeSwitcher({
  enabled,
  isbapXAgent,
  bapxAgentModes,
  onModeChange,
}: bapXAgentModeSwitcherProps) {
  if (!enabled || !(isStagingMode() || isLocalMode()) || !isbapXAgent) return null;
...
export const ChatInput = memo(forwardRef<ChatInputHandles, ChatInputProps>(
  (
    ...
    const [isModeDismissing, setIsModeDismissing] = useState(false);    // bapX Agent Modes feature flag
    const ENABLE_SUNA_AGENT_MODES = false;
    const [bapxAgentModes, setbapXAgentModes] = useState<'adaptive' | 'autonomous' | 'chat'>('adaptive');
    ...
    // Check if selected agent is bapX based on agent data
    // While loading, default to bapX (assume bapX is the default agent)
    const selectedAgent = agents.find(agent => agent.agent_id === selectedAgentId);
    const bapxAgent = agents.find(agent => agent.metadata?.is_bapx_default === true);
    const isbapXAgent = isLoadingAgents 
        ? true // Show bapX modes while loading
        : (selectedAgent?.metadata?.is_bapx_default || (!selectedAgentId && bapxAgent !== undefined) || false);
    ...
    const leftControls = useMemo(() => (
      <div className="flex items-center gap-1 sm:gap-1.5 min-w-0 flex-shrink overflow-visible">
        ...
        <div className="hidden sm:block">
          <bapXAgentModeSwitcher
            enabled={ENABLE_SUNA_AGENT_MODES}
            isbapXAgent={isbapXAgent}
            bapxAgentModes={bapxAgentModes}
            onModeChange={setbapXAgentModes}
          />
        </div>
    ...
    ), [hideAttachments, loading, disabled, isAgentRunning, isUploading, sandboxId, projectId, messages, isLoggedIn, isFreeTier, quickIntegrations, integrationIcons, handleOpenRegistry, handleOpenPlanModal, threadId, isbapXAgent, bapxAgentModes, onModeDeselect, isModeDismissing, handleModeDeselect]);
