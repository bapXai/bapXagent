...
export function bapXModesPanel({ 
  selectedMode, 
  onModeSelect, 
  onSelectPrompt, 
  isMobile = false,
  ...
  const getTranslatedPrompts = (modeId: string): SamplePrompt[] => {
    ...
    for (let index = 0; index < maxPrompts; index++) {
      ...
      try {
        ...
        if (!translatedText || translatedText === `bapx.${key}` || translatedText.startsWith('bapx.prompts.') || translatedText.includes(modeId)) {
  ...
  const selectTemplate = usebapXModesStore((state) => state.selectTemplate);
  const selectOutputFormat = usebapXModesStore((state) => state.selectOutputFormat);
  const selectCharts = usebapXModesStore((state) => state.selectCharts);
  const selectDocsType = usebapXModesStore((state) => state.selectDocsType);
  const selectImageStyle = usebapXModesStore((state) => state.selectImageStyle);
  const selectCanvasAction = usebapXModesStore((state) => state.selectCanvasAction);
  const selectVideoStyle = usebapXModesStore((state) => state.selectVideoStyle);
  const storeSetSelectedMode = usebapXModesStore((state) => state.setSelectedMode);
          ...
          {modes.map((mode) => {
            ...
            return (
              <motion.button
                ...
                className={cn(
                  // Base button styles matching bapX design
