...
export function AgentStartInput({
  ...
  const tbapX = useTranslations('bapx');
  ...
  const resolvedPlaceholder = placeholder || (variant === 'hero' ? tbapX('describeTask') : t('describeWhatYouNeed'));
  ...
  return (
    <>
      ...
      {/* bapX Modes Panel - Always show for hero variant (LP), otherwise check isbapXAgent */}
      {showModesPanel && (variant === 'hero' || isbapXAgent) && (
